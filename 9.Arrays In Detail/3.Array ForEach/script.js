...
//Cara biasa
for(let i=0;i<names.length;i++){ //kalau method for loop biasa kita kena tambah variable baru which in this case is "i"
...
//Example 1                     
names.forEach((name,index)=>{ //Parameter first tu untuk elements dalam array tu.Parameter second tu is index according to the elements respectively,kira dia akan print index sekali.Boleh je nak letak nama apa2 untuk parameter tu
...
numbers.forEach((number)=>{ //tak bubuh index pun takpe janji bubuh parameter first.Boleh je letak nama apa2 untuk parameter
    sum+=number;
});
